window.closeAllDrawers = _callback => {
	...
	bodyClasses.forEach(bodyClass => {
		if (bodyClass.match('drawer-open') || bodyClass.match('drawer-search-open') || bodyClass.match('drawer-cart-open') || bodyClass.match('drawer-menu-open') || bodyClass.match('drawer-mobile-open') || bodyClass.match('drawer-filters-open') || bodyClass.match('drawer-collection-header-description-open') || bodyClass.match('drawer-quick-add-open')) {
...
window.openQuickAddDrawer = () => {
	const documentBody = document.querySelector('body');
	if (documentBody.classList.contains('drawer-quick-add-open')) {
		window.closeAllDrawers(() => {
			documentBody.classList.add('drawer-quick-add-open', 'scroll-lock');
		});
	} else {
		documentBody.classList.add('drawer-quick-add-open', 'scroll-lock');
	}
